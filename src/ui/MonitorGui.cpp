...
void MonitorGui::drawMonitor() const {
    ...
    ImGui::SameLine();
    bool effects = controls_->showEffects.load(std::memory_order_relaxed);
    if (ImGui::Checkbox("effects [e]", &effects)) controls_->showEffects.store(effects, std::memory_order_relaxed);
