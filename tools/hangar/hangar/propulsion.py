...
_ALT_FT = (-10000.0, 0.0, 10000.0, 20000.0, 30000.0, 40000.0, 50000.0, 60000.0)
...
def turbofan_lapse(mach, h_m, tr=1.05, wet=False):
    """Installed thrust over sea-level-static maximum (wet) thrust of a
    low-bypass mixed-flow turbofan (Mattingly, Heiser & Pratt, "Aircraft
    Engine Design", 2002, eq. 2.54): with theta0, delta0 the total
    temperature and pressure ratios, maximum (wet) power
        delta0 (1 - 3.5 (theta0 - TR) / theta0)
    and military (dry) power
        0.6 delta0 (1 - 3.8 (theta0 - TR) / theta0),
    the bracket 1 below the throttle ratio TR."""
    t, p = _isa(h_m)
    f = 1.0 + 0.2 * mach * mach
    theta0, delta0 = t / 288.15 * f, p / 101325.0 * f ** 3.5
    over = max(theta0 - tr, 0.0) / theta0
    return delta0 * (1.0 - 3.5 * over) if wet else 0.6 * delta0 * (1.0 - 3.8 * over)
...
def turbofan_tables(engine):
    ...
    for m in _MACH:
        ...
        for h_ft in _ALT_FT:
            ...
            mil.append(max(turbofan_lapse(m, h, tr) / mil0, 0.0) if h_ft < 60000 else 0.0)
            aug.append(max(turbofan_lapse(m, h, tr, wet=True) / wet0, 0.0) if h_ft < 60000 else 0.0)
...
<!-- Generated by hangar: a turbofan of %.1f kN dry%s, bypass ratio %.2f.
     Thrust lapse: Mattingly, Heiser & Pratt, Aircraft Engine Design (2002),
     eq. 2.54, low-bypass mixed-flow turbofan, throttle ratio %.2f. -->
