...
| `vehicle.capabilities()`, `capability_status(id)` | what the vehicle offers: `fsim.Capability(id, version, kind, interactions, level, axes, terminating, needs_target, behavior, parameters)` with `fsim.Parameter(name, unit, min, max, default, optional)`; ([control.md](control.md#capabilities-and-activities)) |
| `vehicle.submit(level, **fields, source=, range=, min_version=)`, `submit_behavior(id, target=, points=, **params)`, `submit_support(kind, **fields)` ("gear", "flaps", "wheel_brakes", "speedbrake", "pitch_trim"; `fsim.SUPPORT_FIELDS`) | NEW: an `fsim.Activity`, or `fsim.Rejected` (an `fsim.Error` with `.reason` - "authority_held", "out_of_range", ... - and `.other`); `source` is `fsim.Source.POLICY` / `AUTOPILOT` / `OVERRIDE`, `range` `fsim.RangePolicy.CLAMP` / `REJECT` / `NONE` |
