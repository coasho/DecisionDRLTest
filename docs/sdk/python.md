...
| `vehicle.submit(level, **fields, source=, range=, min_version=)`, `submit_behavior(id, target=, points=, **params)` | NEW: an `fsim.Activity`, or `fsim.Rejected` (an `fsim.Error` with `.reason` - "authority_held", "out_of_range", ... - and `.other`); `source` is `fsim.Source.POLICY` / `AUTOPILOT` / `OVERRIDE`, `range` `fsim.RangePolicy.CLAMP` / `REJECT` / `NONE` |
